...
let html = fs.readFileSync('scratch/zip_extract/index.html', 'utf8');
console.log('Original base index.html size:', html.length, 'lines:', html.split('\n').length);

// 1. Update Section 4 (#act-4) to Orbital Culinary Plate Showcase
const act4Old = html.match(/<!-- =+[\s\S]*?ACT 04[\s\S]*?<\/section>/)?.[0];
const act4New = `  <!-- ================================================================
...
if (act4Old) {
  html = html.replace(act4Old, act4New);
  console.log('Replaced Section 4 successfully!');
}

// 2. Update Section 5 (#act-5) to 2x2 Services Grid with Reveal Images & Checklists
const act5Old = html.match(/<!-- =+[\s\S]*?ACT 05 — SERVICES[\s\S]*?<\/section>/)?.[0];
const act5New = `  <!-- ================================================================
...
if (act5Old) {
  html = html.replace(act5Old, act5New);
  console.log('Replaced Section 5 successfully!');
}

// 3. Update Section 6 (#act-6) to Master Caterer Event Architect and place between Corporate Buffet Banner & Section 7
const act6Old = html.match(/<!-- =+[\s\S]*?ACT 06 — MENU CALCULATOR[\s\S]*?<\/section>/)?.[0];
const act7BannerOld = html.match(/<!-- =+[\s\S]*?ACT 07 — CORPORATE BUFFET FULL-STRETCH[\s\S]*?<\/section>/)?.[0];
...
if (act6Old && act7BannerOld) {
  html = html.replace(act6Old, '');
  html = html.replace(act7BannerOld, act7BannerOld + '\n\n' + act6New);
  console.log('Replaced and repositioned Section 6 successfully!');
}

// 4. Update Section 8 (#act-8) to Executive Culinary Business Card Showcase
const act8Old = html.match(/<!-- =+[\s\S]*?ACT 08 — WEDDING FEAST[\s\S]*?<\/footer>/)?.[0];
const act8New = `  <!-- ================================================================
  ...
  <section id="act-8" class="section-light">
    <div class="container">
...
if (act8Old) {
  html = html.replace(act8Old, act8New + '\n\n<footer class="luxury-footer">');
  console.log('Replaced Section 8 successfully!');
}

fs.writeFileSync('index.html', html);
console.log('Saved pristine enhanced index.html! Size:', html.length, 'Lines:', html.split('\n').length);
